...
As we can see from the first run, we still have a small hiccup: as the size of our input changes, so does the address of _b / blah_, so that they still don't match. However, if we use the new address of _b_ in our next run, it works; the overflow continues, and we overwrite our return address with 4 A's. Our last step is to simple replace those A's with the address of EGG, and we have shell as narnia9!
